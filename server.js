...
function show(data){document.querySelector('#uuid').value=data.uuid;for(const k of ['vless','vmess','trojan','ss'])document.querySelector('#'+k).value=data.paths[k];document.querySelector('#uris').innerHTML=Object.entries(data.uris).map(([k,v])=>'<article class="protocol"><h3>'+k.toUpperCase()+'</h3><div class="qr-wrap"><img class="qr" src="'+data.qr[k]+'" alt="'+k+' QR code"></div><div class="uri">'+v+'</div><button class="secondary copy" onclick="navigator.clipboard.writeText('+JSON.stringify(v)+')">Copy URI</button></article>').join('')}
