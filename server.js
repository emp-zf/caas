...
setInterval(()=>{ensureNetworkTests();refreshNetwork().then(syncNetworkSummaryTheme);renderNetworkTests();renderEdgeNetworkCards()},30000);setTimeout(()=>{ensureNetworkTests();renderEdgeNetworkCards();syncNetworkSummaryTheme()},1000);new MutationObserver(()=>{syncNetworkTheme();syncNetworkSummaryTheme()}).observe(document.documentElement,{attributes:true,attributeFilter:['data-theme']});</script></body></html>`;
