...
function compactUriCards(){const columns=window.innerWidth<=800?'1fr':'120px minmax(0,1fr) auto';document.querySelectorAll('.protocol').forEach(card=>{card.style.gridTemplateColumns=columns})}
function renderLogs(logs){const output=document.querySelector('#logs');if(!output)return;output.textContent=logs.join('\\n');output.scrollTop=output.scrollHeight}
async function load(){try{show(await api('/api/config'));compactUriCards();renderLogs((await api('/api/logs')).logs)}catch(e){document.querySelector('#status').textContent=e.message}}
