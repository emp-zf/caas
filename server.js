...
async function load(){try{const data=await api('/api/config');show(data);compactUriCards();initializeConfigModal();addXhttpConfig();addNetworkCard();ensureNetworkTests();document.querySelector('#xhttp').value=data.paths.xhttp;bindConfigForm();arrangeActions();await refreshNetwork();syncNetworkSummaryTheme();renderNetworkTests();renderLogs((await api('/api/logs')).logs)}catch(e){document.querySelector('#status').textContent=e.message}}
...
setInterval(()=>{ensureNetworkTests();refreshNetwork().then(syncNetworkSummaryTheme);renderNetworkTests();renderEdgeNetworkCards()},30000);setTimeout(()=>{ensureNetworkTests();renderEdgeNetworkCards();syncNetworkSummaryTheme();syncModalTheme()},1000);new MutationObserver(()=>{syncNetworkTheme();syncNetworkSummaryTheme();syncModalTheme()}).observe(document.documentElement,{attributes:true,attributeFilter:['data-theme']});new MutationObserver(syncModalTheme).observe(document.body,{childList:true,subtree:true});</script></body></html>`;
