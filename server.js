...
function show(data){document.querySelector('#uuid').value=data.uuid;for(const k of ['vless','vmess','trojan','ss'])document.querySelector('#'+k).value=data.paths[k];document.querySelector('#uris').innerHTML=Object.entries(data.uris).map(([k,v])=>'<article class="protocol"><h3>'+k.toUpperCase()+'</h3><div class="uri">'+v+'</div><div style="display:flex;gap:8px"><button class="secondary copy" style="flex:1;width:auto" onclick="openQr('+JSON.stringify(data.qr[k])+','+JSON.stringify(k)+')">QR code</button><button class="secondary copy" style="flex:1;width:auto" onclick="navigator.clipboard.writeText('+JSON.stringify(v)+')">Copy URI</button></div></article>').join('')}
